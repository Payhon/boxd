...
export const localControlFlow = `
flowchart TB
  accTitle: boxd 本地沙盒调用流程
  accDescr: Upstash Box SDK 请求进入 boxd HTTP API，控制面持久化状态并启动独立 worker，随后通过 libkrun 在 Linux microVM 中运行 box-agent。

  SDK["@upstash/box SDK"]
  API["boxd HTTP API"]
  DB[("SQLite / PostgreSQL / MySQL")]
  Worker["boxd __vmm-worker"]
  Hypervisor["libkrun v1.19.4 · HVF / KVM"]
  VM["Linux microVM"]
  Agent["box-agent"]
...
<Flowchart
  chart={localControlFlow}
  title="从 SDK 请求到隔离 microVM"
  description="控制面负责状态，worker 与 microVM 承载不可信工作负载"
/>
